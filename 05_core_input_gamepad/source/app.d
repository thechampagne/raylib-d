...
enum const(char)* XBOX360_LEGACY_NAME_ID = "Xbox Controller";
version(PLATFORM_RPI)
{
	enum const(char)* XBOX360_NAME_ID = "Microsoft X-Box 360 pad";
    enum const(char)* PS3_NAME_ID     = "PLAYSTATION(R)3 Controller";
}
else
{
	enum const(char)* XBOX360_NAME_ID = "Xbox 360 Controller";
    enum const(char)* PS3_NAME_ID     = "PLAYSTATION(R)3 Controller";
}
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            if (IsGamepadAvailable(0))
            {
                ...
                if (GetGamepadButtonPressed() != -1) DrawText(TextFormat("DETECTED BUTTON: %i", GetGamepadButtonPressed()), 10, 430, 10, Colors.RED);
